"use strict";(()=>{var v=class{constructor(){this.events=new Map}on(e,t){this.events.has(e)||this.events.set(e,new Set),this.events.get(e).add(t)}off(e,t){this.events.get(e)?.delete(t)}emit(e){this.events.get(e)?.forEach(t=>t())}},m=class o extends v{constructor(){super(...arguments);this.state={isScrolled:!1,menuOpen:!1,activeSection:"inicio"}}static getInstance(){return o.instance||(o.instance=new o),o.instance}setScrolled(t){this.state.isScrolled!==t&&(this.state.isScrolled=t,this.emit("scroll"))}toggleMenu(){this.state.menuOpen=!this.state.menuOpen,this.emit("menu")}closeMenu(){this.state.menuOpen&&(this.state.menuOpen=!1,this.emit("menu"))}setActiveSection(t){this.state.activeSection!==t&&(this.state.activeSection=t,this.emit("section"))}};var h=class extends v{constructor(){super(...arguments);this.data={name:"",email:"",phone:"",message:""};this.status="idle";this.errors={}}update(t,i){this.data[t]=i,this.errors[t]&&(delete this.errors[t],this.emit("change"))}validate(){return this.errors={},this.data.name.trim()||(this.errors.name="El nombre es obligatorio."),this.data.email.trim()?/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.data.email)||(this.errors.email="Email inv\xE1lido."):this.errors.email="El email es obligatorio.",this.data.message.trim()||(this.errors.message="El mensaje es obligatorio."),this.emit("change"),Object.keys(this.errors).length===0}async submit(){this.validate()&&(this.status="sending",this.emit("status"),await new Promise(t=>setTimeout(t,2e3)),this.status="sent",this.emit("status"))}};var P=/^NEXA-[A-Z0-9]{6,10}$/i,u=class extends v{constructor(){super(...arguments);this.id="";this.status="idle";this.error=""}setId(t){this.id=t.toUpperCase(),this.error="",this.emit("change")}validate(){return this.id.trim()?P.test(this.id.trim())?!0:(this.error="Formato inv\xE1lido. Ejemplo: NEXA-ABC123456",this.emit("change"),!1):(this.error="Por favor, introduce tu ID de dispositivo.",this.emit("change"),!1)}async activate(){this.validate()&&(this.status="validating",this.error="",this.emit("status"),await new Promise(t=>setTimeout(t,2e3)),this.status="success",this.emit("status"))}reset(){this.id="",this.status="idle",this.error="",this.emit("status"),this.emit("change")}};var r=class{el(e,t){let i=document.createElement(e);return t&&(i.className=t),i}html(e,t,i){let a=document.createElement(e);return i&&(a.className=i),a.innerHTML=t,a}svgIcon(e,t=24){let i=document.createElementNS("http://www.w3.org/2000/svg","svg");return i.setAttribute("width",String(t)),i.setAttribute("height",String(t)),i.setAttribute("viewBox","0 0 24 24"),i.setAttribute("fill","none"),i.setAttribute("stroke","currentColor"),i.setAttribute("stroke-width","1.5"),i.setAttribute("stroke-linecap","round"),i.setAttribute("stroke-linejoin","round"),i.innerHTML=e,i}getContainer(){return this.container}};var F=[{label:"Inicio",href:"#inicio"},{label:"Producto",href:"#solucion"},{label:"C\xF3mo funciona",href:"#como-funciona"},{label:"Beneficios",href:"#beneficios"},{label:"Planes",href:"#plan"},{label:"Contacto",href:"#contacto"}],g=class extends r{render(){let e=this.el("header","nav-header");return e.innerHTML=`
      <div class="nav-inner">
        ...
        <nav class="nav-links">
          ${F.map(t=>`<a href="${t.href}" class="nav-link" data-nav>${t.label}</a>`).join("")}
        </nav>
      ...
      <div class="footer-inner">
        <div class="footer-grid">
          ...
          <div class="footer-col">
            <h4>Empresa</h4>
            <div class="footer-links">
              ${R.map(t=>`<a href="${t.h}" class="footer-link" data-nav>${t.l}</a>`).join("")}
            </div>
          </div>
        </div>
        <div class="footer-bottom">
          ...
          <div class="footer-legal">
            ${Y.map(t=>`<a href="#">${t}</a>`).join("")}
          </div>
        </div>
      </div>
    `,this.container=e,e}getNavLinks(){return this.container.querySelectorAll("[data-nav]")}};var f=class extends r{render(){let e=this.el("section","hero-section");return e.id="inicio",e.innerHTML=`
    ...
    `,this.container=e,e}};var _=[{icon:'<path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>',title:"Reduce la soledad",desc:"IA proactiva que detecta patrones de aislamiento y act\xFAa antes de que el usuario lo perciba conscientemente.",color:"#00E5C3"},{icon:'<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z"/><path d="M8 14s1.5 2 4 2 4-2 4-2M9 9h.01M15 9h.01"/>',title:"Bienestar emocional",desc:"Monitorizaci\xF3n del estado emocional con respuestas emp\xE1ticas y personalizadas que mejoran la calidad de vida.",color:"#00CFFF"},{icon:'<circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/>',title:"Recomienda actividades",desc:"Propone planes, talleres, paseos y eventos seg\xFAn los intereses y la ubicaci\xF3n del usuario cada d\xEDa.",color:"#00FFD5"},{icon:'<path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/>',title:"Conecta personas",desc:"Crea microcomunidades de personas con intereses similares. Relaciones reales, no solo digitales.",color:"#00E5C3"},{icon:'<path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 10.8a19.79 19.79 0 01-3.07-8.63A2 2 0 012 .17h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92z"/>',title:"Notifica a familiares",desc:"Alertas inteligentes a familiares sobre el bienestar, sin invadir la privacidad del usuario.",color:"#00CFFF"},{icon:'<polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>',title:"F\xE1cil de usar",desc:"Cero aplicaciones, cero pantallas complejas. Solo la pulsera en el mu\xF1eca y la voz para interactuar.",color:"#00FFD5"}],M=class extends r{render(){let e=this.el("section","section");return e.id="beneficios",e.innerHTML=`
    ...
    `,this.container=e,e}};var z=[{e:"\u{1F499}",t:"Empat\xEDa",d:"Cada decisi\xF3n de dise\xF1o parte de escuchar y comprender las necesidades reales de las personas mayores y sus familias.",grad:"rgba(0,229,195,.15),rgba(0,207,255,.1)",border:"rgba(0,229,195,.2)"},{e:"\u{1F30D}",t:"Inclusi\xF3n",d:"Creemos que la tecnolog\xEDa debe ser para todos. Sin barreras de edad, capacidad digital ni condici\xF3n econ\xF3mica.",grad:"rgba(0,207,255,.15),rgba(0,229,195,.1)",border:"rgba(0,207,255,.2)"},{e:"\u{1F680}",t:"Innovaci\xF3n",d:"IA, IoT y dise\xF1o de vanguardia al servicio del bienestar humano. Innovamos porque creemos que lo mejor est\xE1 por venir.",grad:"rgba(0,229,195,.15),rgba(0,255,213,.1)",border:"rgba(0,229,195,.2)"},{e:"\u{1F331}",t:"Sostenibilidad",d:"Hardware duradero, software actualizable y una huella digital responsable. Construimos para el largo plazo.",grad:"rgba(0,255,213,.15),rgba(0,207,255,.1)",border:"rgba(0,255,213,.2)"},{e:"\u2696\uFE0F",t:"\xC9tica",d:"Privacidad por dise\xF1o, transparencia total y cumplimiento RGPD. Los datos de nuestros usuarios nunca se venden.",grad:"rgba(0,207,255,.15),rgba(0,229,195,.1)",border:"rgba(0,207,255,.2)"}],T=class extends r{render(){let e=this.el("section","section section-alt");return e.id="valores",e.innerHTML=`
      <div class="container">
        <div class="section-center reveal">
    ...
    `,this.container=e,e}};var te='<svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" stroke-linecap="round"/></svg>',ie='<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6L9 17l-5-5" stroke-linecap="round" stroke-linejoin="round"/></svg>',k=class extends r{render(){let e=this.el("section","section");return e.id="activar",e.innerHTML=`
          ...
          <div class="activate-border">
            <div class="activate-card">
              ...
              <div class="activate-success" id="activate-success" style="display:none">
                <div class="activate-success-icon" style="color:var(--bg)">${ie}</div>
                <div>
    ...
    `,this.container=e,this.formEl=e.querySelector("#activate-form"),this.successEl=e.querySelector("#activate-success"),this.inputEl=e.querySelector("#device-id"),this.errorEl=e.querySelector("#device-error"),this.submitBtn=e.querySelector("#activate-btn"),e}getInput(){return this.inputEl}getSubmitBtn(){return this.submitBtn}getResetBtn(){return this.container.querySelector("#activate-reset")}showError(e){let t=this.errorEl.querySelector("#device-error-msg");t.textContent=e,this.errorEl.style.display="flex"}clearError(){this.errorEl.style.display="none"}setValidating(e){this.submitBtn.disabled=e,this.submitBtn.innerHTML=e?`${te} Verificando dispositivo\u2026`:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke-linecap="round" stroke-linejoin="round"/></svg> Activar mi pulsera NEXA'}showSuccess(e){let t=this.container.querySelector("#activated-id");t.textContent=e,this.formEl.style.display="none",this.successEl.style.display="flex"}reset(){this.inputEl.value="",this.clearError(),this.setValidating(!1),this.successEl.style.display="none",this.formEl.style.display="flex"}};var ae='<svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" stroke-linecap="round"/></svg>',se='<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--bg)"><path d="M20 6L9 17l-5-5" stroke-linecap="round" stroke-linejoin="round"/></svg>',A=class extends r{render(){let e=this.el("section","section section-alt");return e.id="contacto",e.innerHTML=`
            ...
            <div class="form-border">
              <div class="form-card">
                ...
                <div class="form-success" id="form-success" style="display:none">
                  <div class="form-success-icon">${se}</div>
    ...
    `,this.container=e,e}getForm(){return this.container.querySelector("#contact-form")}getSubmitBtn(){return this.container.querySelector("#form-submit")}showFieldError(e,t){let i=this.container.querySelector(`#err-${e}`);if(!i)return;let a=i.querySelector("span");a.textContent=t,i.style.display="flex"}clearErrors(){this.container.querySelectorAll(".input-error").forEach(e=>e.style.display="none")}setSending(e){let t=this.getSubmitBtn();t.disabled=e,t.innerHTML=e?`${ae} Enviando\u2026`:"Enviar mensaje"}showSuccess(){let e=this.container.querySelector("#contact-form"),t=this.container.querySelector("#form-success");e.style.display="none",t.style.display="flex"}};var C=class{constructor(e,t,i,a){this.model=e;this.headerView=t;this.footerView=i;this.smoothScroll=a}init(){this.bindScroll(),this.bindBurger(),this.bindNavLinks(this.headerView.getNavLinks()),this.bindNavLinks(this.footerView.getNavLinks()),this.bindModelEvents()}bindScroll(){window.addEventListener("scroll",()=>{this.model.setScrolled(window.scrollY>20)},{passive:!0})}bindBurger(){this.headerView.getBurger().addEventListener("click",()=>{this.model.toggleMenu()})}bindNavLinks(e){e.forEach(t=>{t.addEventListener("click",i=>{i.preventDefault();let a=t.getAttribute("href");if(a?.startsWith("#")){let s=document.querySelector(a);s&&this.smoothScroll.scrollToEl(s),this.model.closeMenu()}})})}bindModelEvents(){this.model.on("scroll",()=>{this.headerView.setScrolled(this.model.state.isScrolled)}),this.model.on("menu",()=>{this.headerView.setMenuOpen(this.model.state.menuOpen)})}};var $=class{constructor(){this.y=0;this.targetY=0;this.ease=.085;this.active=!1;this.tops=[]}init(){if(this.y=this.targetY=window.scrollY,window.matchMedia("(pointer: coarse)").matches)return;this.active=!0,document.documentElement.style.scrollBehavior="auto",this.calcTops(),window.addEventListener("resize",()=>this.calcTops(),{passive:!0});let e=0,t=0;window.addEventListener("wheel",a=>{a.preventDefault(),clearTimeout(e),t+=a.deltaY,this.targetY=this.clamp(this.targetY+a.deltaY),e=window.setTimeout(()=>{let s=t>=0?1:-1;t=0,this.snap(s)},150)},{passive:!1});let i=()=>{let a=this.targetY-this.y;Math.abs(a)>.1&&(this.y+=a*this.ease,window.scrollTo(0,this.y)),requestAnimationFrame(i)};requestAnimationFrame(i)}scrollToEl(e){this.targetY=this.clamp(this.absTop(e)),this.active||e.scrollIntoView({behavior:"smooth"})}snap(e){let t=this.tops;if(!t.length)return;let i=this.targetY,a=window.innerHeight,s=t[0];for(let d of t)d<=i+4&&(s=d);let n=t.find(d=>d>i+4)??s,l=i-s,c=n-i;e>0?this.targetY=this.clamp(l>a*.38?n:s):this.targetY=this.clamp(c>a*.38?s:n)}calcTops(){this.tops=Array.from(document.querySelectorAll(".hero-section, .section")).map(e=>this.absTop(e)).sort((e,t)=>e-t)}absTop(e){let t=0,i=e;for(;i;)t+=i.offsetTop,i=i.offsetParent;return t}clamp(e){return Math.max(0,Math.min(e,document.documentElement.scrollHeight-window.innerHeight))}};var X=["#00E5C3","#00CFFF","#00FFD5","rgba(0,207,255,0.6)"],H=class{constructor(e){this.canvas=e;this.particles=[];this.animId=0;this.loop=()=>{let{ctx:e,canvas:t,particles:i}=this;e.clearRect(0,0,t.width,t.height);for(let a=0;a<i.length;a++){let s=i[a];s.x+=s.vx,s.y+=s.vy,(s.x<0||s.x>t.width)&&(s.vx*=-1),(s.y<0||s.y>t.height)&&(s.vy*=-1),e.beginPath(),e.arc(s.x,s.y,s.r,0,Math.PI*2),e.fillStyle=s.color,e.globalAlpha=s.alpha,e.fill();for(let n=a+1;n<i.length;n++){let l=i[n],c=s.x-l.x,d=s.y-l.y,p=Math.sqrt(c*c+d*d);p<120&&(e.beginPath(),e.moveTo(s.x,s.y),e.lineTo(l.x,l.y),e.strokeStyle="#00E5C3",e.globalAlpha=(1-p/120)*.12,e.lineWidth=.5,e.stroke())}}e.globalAlpha=1,this.animId=requestAnimationFrame(this.loop)};this.ctx=e.getContext("2d")}init(){this.resize(),window.addEventListener("resize",()=>this.resize(),{passive:!0});for(let e=0;e<120;e++)this.particles.push(this.spawn());this.loop()}destroy(){cancelAnimationFrame(this.animId)}resize(){this.canvas.width=this.canvas.offsetWidth,this.canvas.height=this.canvas.offsetHeight}spawn(){return{x:Math.random()*this.canvas.width,y:Math.random()*this.canvas.height,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4,r:Math.random()*2+.5,alpha:Math.random()*.6+.2,color:X[Math.floor(Math.random()*X.length)]}}};var V=class{constructor(){this.observer=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&(t.target.classList.add("visible"),this.observer.unobserve(t.target))})},{threshold:.08,rootMargin:"0px 0px -50px 0px"})}observe(){document.querySelectorAll(".reveal, .reveal-left, .reveal-right").forEach(e=>this.observer.observe(e))}};var B=class{constructor(){this.observer=new IntersectionObserver(e=>{e.forEach(t=>{if(t.isIntersecting){let i=t.target,a=parseFloat(i.dataset.counter??"0"),s=i.dataset.suffix??"";this.animate(i,a,s),this.observer.unobserve(i)}})},{threshold:.5})}observe(){document.querySelectorAll("[data-counter]").forEach(e=>this.observer.observe(e))}animate(e,t,i){let s=Date.now(),n=()=>{let l=Date.now()-s,c=Math.min(l/1800,1),d=1-Math.pow(1-c,3),p=t*d;e.textContent=(Number.isInteger(t)?Math.round(p):parseFloat(p.toFixed(1)))+i,c<1&&requestAnimationFrame(n)};requestAnimationFrame(n)}};var I=class{constructor(e,t){this.model=e;this.view=t}init(){let e=this.view.getForm();e.addEventListener("submit",async t=>{t.preventDefault(),this.view.clearErrors();let i=new FormData(e);if(this.model.update("name",String(i.get("name")??"")),this.model.update("email",String(i.get("email")??"")),this.model.update("phone",String(i.get("phone")??"")),this.model.update("message",String(i.get("message")??"")),!this.model.validate()){this.model.errors.name&&this.view.showFieldError("name",this.model.errors.name),this.model.errors.email&&this.view.showFieldError("email",this.model.errors.email),this.model.errors.message&&this.view.showFieldError("msg",this.model.errors.message);return}this.view.setSending(!0),await this.model.submit(),this.view.setSending(!1),this.model.status==="sent"&&this.view.showSuccess()})}};var N=class{constructor(e,t){this.model=e;this.view=t}init(){let e=this.view.getInput(),t=this.view.getSubmitBtn(),i=this.view.getResetBtn();e.addEventListener("input",()=>{this.model.setId(e.value),e.value=this.model.id,this.view.clearError()}),e.addEventListener("keydown",a=>{a.key==="Enter"&&t.click()}),t.addEventListener("click",async()=>{if(this.view.clearError(),!this.model.validate()){this.view.showError(this.model.error);return}this.view.setValidating(!0),await this.model.activate(),this.view.setValidating(!1),this.model.status==="success"&&this.view.showSuccess(this.model.id)}),i.addEventListener("click",()=>{this.model.reset(),this.view.reset()})}};var oe=()=>{let o=document.createElement("div");return o.className="section-divider",o},q=class{constructor(){this.appModel=m.getInstance();this.formModel=new h;this.deviceModel=new u;this.headerView=new g;this.heroView=new f;this.problemView=new w;this.solutionView=new y;this.howItWorksView=new E;this.ecosystemView=new x;this.benefitsView=new M;this.planView=new L;this.socialImpactView=new S;this.valuesView=new T;this.activateDeviceView=new k;this.contactView=new A;this.footerView=new b}init(){let e=document.getElementById("app");if(!e)throw new Error("#app element not found");e.appendChild(this.headerView.render());let t=document.createElement("main"),i=[this.heroView,this.problemView,this.solutionView,this.howItWorksView,this.ecosystemView,this.benefitsView,this.planView,this.socialImpactView,this.valuesView,this.activateDeviceView,this.contactView];i.forEach((n,l)=>{t.appendChild(n.render()),l<i.length-1&&t.appendChild(oe())}),e.appendChild(t),e.appendChild(this.footerView.render());let a=new $;a.init(),this.heroView.getScrollButtons().forEach(n=>{n.addEventListener("click",()=>{let l=n.dataset.scroll,c=l?document.querySelector(l):null;c&&a.scrollToEl(c)})}),this.planView.getCTA()?.addEventListener("click",()=>{let n=document.querySelector("#contacto");n&&a.scrollToEl(n)}),new C(this.appModel,this.headerView,this.footerView,a).init(),new V().observe(),new B().observe(),new I(this.formModel,this.contactView).init(),new N(this.deviceModel,this.activateDeviceView).init(),requestAnimationFrame(()=>{let n=this.heroView.getCanvas();n&&new H(n).init()})}};document.addEventListener("DOMContentLoaded",()=>{new q().init()});})();
