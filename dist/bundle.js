"use strict";(()=>{var p=class{constructor(){this.events=new Map}on(e,t){this.events.has(e)||this.events.set(e,new Set),this.events.get(e).add(t)}off(e,t){this.events.get(e)?.delete(t)}emit(e){this.events.get(e)?.forEach(t=>t())}},h=class o extends p{constructor(){super(...arguments);this.state={isScrolled:!1,menuOpen:!1,activeSection:"inicio"}}static getInstance(){return o.instance||(o.instance=new o),o.instance}setScrolled(t){this.state.isScrolled!==t&&(this.state.isScrolled=t,this.emit("scroll"))}toggleMenu(){this.state.menuOpen=!this.state.menuOpen,this.emit("menu")}closeMenu(){this.state.menuOpen&&(this.state.menuOpen=!1,this.emit("menu"))}setActiveSection(t){this.state.activeSection!==t&&(this.state.activeSection=t,this.emit("section"))}};var u=class extends p{constructor(){super(...arguments);this.data={name:"",email:"",phone:"",message:""};this.status="idle";this.errors={}}update(t,i){this.data[t]=i,this.errors[t]&&(delete this.errors[t],this.emit("change"))}validate(){return this.errors={},this.data.name.trim()||(this.errors.name="El nombre es obligatorio."),this.data.email.trim()?/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.data.email)||(this.errors.email="Email inv\xE1lido."):this.errors.email="El email es obligatorio.",this.data.message.trim()||(this.errors.message="El mensaje es obligatorio."),this.emit("change"),Object.keys(this.errors).length===0}async submit(){this.validate()&&(this.status="sending",this.emit("status"),await new Promise(t=>setTimeout(t,2e3)),this.status="sent",this.emit("status"))}};var O=/^NEXA-[A-Z0-9]{6,10}$/i,g=class extends p{constructor(){super(...arguments);this.id="";this.status="idle";this.error=""}setId(t){this.id=t.toUpperCase(),this.error="",this.emit("change")}validate(){return this.id.trim()?O.test(this.id.trim())?!0:(this.error="Formato inv\xE1lido. Ejemplo: NEXA-ABC123456",this.emit("change"),!1):(this.error="Por favor, introduce tu ID de dispositivo.",this.emit("change"),!1)}async activate(){this.validate()&&(this.status="validating",this.error="",this.emit("status"),await new Promise(t=>setTimeout(t,2e3)),this.status="success",this.emit("status"))}reset(){this.id="",this.status="idle",this.error="",this.emit("status"),this.emit("change")}};var r=class{el(e,t){let i=document.createElement(e);return t&&(i.className=t),i}html(e,t,i){let a=document.createElement(e);return i&&(a.className=i),a.innerHTML=t,a}svgIcon(e,t=24){let i=document.createElementNS("http://www.w3.org/2000/svg","svg");return i.setAttribute("width",String(t)),i.setAttribute("height",String(t)),i.setAttribute("viewBox","0 0 24 24"),i.setAttribute("fill","none"),i.setAttribute("stroke","currentColor"),i.setAttribute("stroke-width","1.5"),i.setAttribute("stroke-linecap","round"),i.setAttribute("stroke-linejoin","round"),i.innerHTML=e,i}getContainer(){return this.container}};var z=[{label:"Inicio",href:"#inicio"},{label:"Producto",href:"#solucion"},{label:"C\xF3mo funciona",href:"#como-funciona"},{label:"Beneficios",href:"#beneficios"},{label:"Planes",href:"#plan"},{label:"Contacto",href:"#contacto"}],b=class extends r{render(){let e=this.el("header","nav-header");return e.innerHTML=`
      <div class="nav-inner">
        ...
        <div class="footer-grid">
          ...
          <div class="footer-col">
            ...
            <div class="footer-links">
              ${Y.map(t=>`<a href="${t.h}" class="footer-link" data-nav>${t.l}</a>`).join("")}
            </div>
          </div>
          <div class="footer-col">
            <h4>Empresa</h4>
    ...
    `,this.container=e,e}};var ee=["Pulsera NEXA incluida","IA personalizada 24/7","Soporte t\xE9cnico dedicado","Actualizaciones autom\xE1ticas","Gesti\xF3n de actividades sociales","Mantenimiento preventivo","Alertas a familiares","Comunidad NEXA premium"],te='<svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4l3 3 5-6" stroke="#0A0A0A" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',S=class extends r{render(){let e=this.el("section","section section-alt");return e.id="plan",e.innerHTML=`
    ...
    `,this.container=e,e}};var ae='<svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" stroke-linecap="round"/></svg>',se='<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6L9 17l-5-5" stroke-linecap="round" stroke-linejoin="round"/></svg>',A=class extends r{render(){let e=this.el("section","section");return e.id="activar",e.innerHTML=`
          ...
          <div class="activate-border">
            <div class="activate-card">
              ...
              <div class="activate-success" id="activate-success" style="display:none">
                <div class="activate-success-icon" style="color:var(--bg)">${se}</div>
                <div>
    ...
    `,this.container=e,this.formEl=e.querySelector("#activate-form"),this.successEl=e.querySelector("#activate-success"),this.inputEl=e.querySelector("#device-id"),this.errorEl=e.querySelector("#device-error"),this.submitBtn=e.querySelector("#activate-btn"),e}getInput(){return this.inputEl}getSubmitBtn(){return this.submitBtn}getResetBtn(){return this.container.querySelector("#activate-reset")}showError(e){let t=this.errorEl.querySelector("#device-error-msg");t.textContent=e,this.errorEl.style.display="flex"}clearError(){this.errorEl.style.display="none"}setValidating(e){this.submitBtn.disabled=e,this.submitBtn.innerHTML=e?`${ae} Verificando dispositivo\u2026`:'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke-linecap="round" stroke-linejoin="round"/></svg> Activar mi pulsera NEXA'}showSuccess(e){let t=this.container.querySelector("#activated-id");t.textContent=e,this.formEl.style.display="none",this.successEl.style.display="flex"}reset(){this.inputEl.value="",this.clearError(),this.setValidating(!1),this.successEl.style.display="none",this.formEl.style.display="flex"}};var oe='<svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" stroke-linecap="round"/></svg>',re='<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--bg)"><path d="M20 6L9 17l-5-5" stroke-linecap="round" stroke-linejoin="round"/></svg>',C=class extends r{render(){let e=this.el("section","section section-alt");return e.id="contacto",e.innerHTML=`
            ...
            <div class="form-border">
              <div class="form-card">
                ...
                <div class="form-success" id="form-success" style="display:none">
                  <div class="form-success-icon">${re}</div>
